...
def get_masked_sent_lm_output(bert_config,
  ...
  independent_sent_embeddings = gather_indexes(cur_sent_reps_doc_unmask, sent_masked_positions)
...
def get_seq_rep_from_bert(bert_model):
  ...
  siamese_input_tensor = tf.layers.dense(siamese_input_tensor, units=hidden_size, activation=tf.nn.relu)
  normalized_siamese_input_tensor = tf.nn.l2_normalize(siamese_input_tensor, axis=1)
...
def learn_sent_reps_normal_loop(dual_encoder_config, is_training, train_mode,
  ...
  # Generate document level input masks on each sentence based on the word level input mask information.
  ...
  # Shape of masked_lm_example_loss_doc_1 is [max_doc_length_by_sentence, batch * max_predictions_per_seq].
  for sent_index in range(0, dual_encoder_config.encoder_config.loop_sent_number_per_doc):

    (input_sent_reps_doc_1, input_mask_doc_level_1, masked_lm_loss_doc_1, masked_lm_example_loss_doc_1, masked_lm_weights_doc_1) = \
    get_sent_reps_masks_normal_loop(
  ...
  if (train_mode == constants.TRAIN_MODE_PRETRAIN or train_mode == constants.TRAIN_MODE_JOINT_TRAIN):
    # Output Shape:  [batch * max_predictions_per_seq, loop_sent_number_per_doc].
    masked_lm_example_loss_doc_1 = tf.stack(masked_lm_example_loss_doc_1, axis=1)
    masked_lm_example_loss_doc_2 = tf.stack(masked_lm_example_loss_doc_2, axis=1)
