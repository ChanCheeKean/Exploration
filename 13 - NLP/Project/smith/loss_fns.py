...
def _pointwise_cosine(encodings_1, encodings_2):
  ...
  Args:
    encodings_1: A 2-D tensor of (left) encodings with shape [batch size, encoding dim].
    encodings_2: A 2-D tensor of (right) encodings with shape [batch size, encoding dim].
  Returns:
...
def get_prediction_loss_cosine(input_tensor_1,
  ...
  Args:
    input_tensor_1: The Tensor with shape [batch_size, embed_size] to denote the left input text.
    input_tensor_2:  The Tensor with shape [batch_size, embed_size] to denote the right input text.
    labels: Float tensor with shape [batch_size]. The ground truth labels to denote whether two documents are matched.
    similarity_score_amplifier: The amplifier to increase the logits value, so that sigmoid(logits) is closer to 0 or 1.
    neg_to_pos_example_ratio: The ratio to compensate when we have more negative examples.
  ...
  with tf.variable_scope("loss/text_pair_matching"):
    ...
    # To compensate when we have way more neg examples than pos examples and to compensate for larger masked lm loss.
    # use weights_2 to make sure the weight for neg examples is 1, not 0.
